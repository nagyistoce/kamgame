...
namespace Microsoft.Xna.Framework.Graphics.PackedVector
{
    public struct NormalizedShort4 : IPackedVector<ulong>, IEquatable<NormalizedShort4>
    {
        ...
        public static bool operator !=(NormalizedShort4 a, NormalizedShort4 b) { return !a.Equals(b); }
        ...
        private static ulong PackInFour(float vectorX, float vectorY, float vectorZ, float vectorW)
        {
            ...
            var word3 =
                (ulong)
                    (((int)MathHelper.Clamp((float)Math.Round(vectorY * maxPos), minNeg, maxPos) & 0xFFFFFFFF) << 0x10);
            var word2 =
                (ulong)
                    (((int)MathHelper.Clamp((float)Math.Round(vectorZ * maxPos), minNeg, maxPos) & 0xFFFFFFFF) << 0x20);
            var word1 =
                (ulong)
                    (((int)MathHelper.Clamp((float)Math.Round(vectorW * maxPos), minNeg, maxPos) & 0xFFFFFFFF) << 0x30);

            return (word4 | word3 | word2 | word1);
        }

        void IPackedVector.PackFromVector4(Vector4 vector) { short4Packed = PackInFour(vector.X, vector.Y, vector.Z, vector.W); }
