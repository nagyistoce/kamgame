...
namespace Microsoft.Xna.Framework.Net
{
    ...
    {
        private readonly BackgroundWorker MGServerWorker = new BackgroundWorker();
        private bool done;
        private NetServer peer;
        private readonly NetworkSession session;
        private readonly AvailableNetworkSession availableSession;
        private string myLocalAddress = string.Empty;
        private IPEndPoint myLocalEndPoint;
        private readonly Dictionary<long, NetConnection> pendingGamers = new Dictionary<long, NetConnection>();
        ...
        static MonoGamerPeer()
        {
            ...
            var assembly = Assembly.GetAssembly(Game.Instance.GetType());
            if (assembly != null)
            {
                var objects = assembly.GetCustomAttributes(typeof (GuidAttribute), false);
                if (objects.Length > 0)
                {
                    applicationIdentifier = ((GuidAttribute)objects[0]).Value;
                }
            }
#else
        ...
        public MonoGamerPeer(NetworkSession session, AvailableNetworkSession availableSession)
        {
            ...
            online = this.session.SessionType == NetworkSessionType.PlayerMatch;
            ...
            MGServerWorker.DoWork += MGServer_DoWork;
            MGServerWorker.RunWorkerCompleted += MGServer_RunWorkerCompleted;
        ...
        internal void ShutDown() { MGServerWorker.CancelAsync(); }
        ...
        private void MGServer_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            var adr = IPAddress.Parse(myLocalAddress);
            ...
            else
            {
                if (online)
                {
                    var ipaddr = NetUtility.Resolve(masterServer);
                    if (ipaddr != null)
                    {
                        m_masterServer = new IPEndPoint(ipaddr, masterserverport);
                        var localMe = session.LocalGamers[0];
                        ...
                        var propertyData = new int[session.SessionProperties.Count * 2];
                        NetworkSessionProperties.WriteProperties(session.SessionProperties, propertyData);
                        for (var x = 0; x < propertyData.Length; x++)
                        {
                ...
                while ((msg = peer.ReadMessage()) != null)
                {
                    switch (msg.MessageType)
                    {
                        ...
                        case NetIncomingMessageType.StatusChanged:
                            ...
                            {
#if !WINDOWS_PHONE
                                Game.Instance.Log(NetUtility.ToHexString(msg.SenderConnection.RemoteUniqueIdentifier) +
                                    " disconnected! from " + msg.SenderEndpoint);
#endif
                                var cgj = new CommandGamerLeft(msg.SenderConnection.RemoteUniqueIdentifier);
                                var cmde = new CommandEvent(cgj);
                                ...
                                if (!pendingGamers.ContainsKey(msg.SenderConnection.RemoteUniqueIdentifier))
                                {
                                    ...
                                    Game.Instance.Log(
                                        NetUtility.ToHexString(msg.SenderConnection.RemoteUniqueIdentifier) +
                                            " connected! from " + msg.SenderEndpoint);
#endif
...
#if !WINDOWS_PHONE
                                    Game.Instance.Log(
                                        "Already have a connection for that user, this is probably due to both NAT intro requests working");
#endif
                                }
                            }
                        ...
                        case NetIncomingMessageType.Data:
                            ...
                            var mt = (NetworkMessageType)msg.ReadByte();
                            switch (mt)
                            {
                                case NetworkMessageType.Data:
                                    var data = new byte[msg.LengthBytes - 1];
                                    msg.ReadBytes(data, 0, data.Length);
                                    var cme =
                                        new CommandEvent(
                                            new CommandReceiveData(msg.SenderConnection.RemoteUniqueIdentifier,
                                                data));
                                ...
                                case NetworkMessageType.Introduction:
                                    ...
                                    try
                                    {
                                        var endPoint = ParseIPEndPoint(introductionAddress);

                                        if (myLocalEndPoint.ToString() != endPoint.ToString() &&
                                            !AlreadyConnected(endPoint))
                                        {
#if !WINDOWS_PHONE
                                    ...
                                    {
#if !WINDOWS_PHONE
                                        Game.Instance.Log("Error parsing Introduction: " + introductionAddress + " : " +
                                            exc.Message);
#endif
                                    }
...
#if !WINDOWS_PHONE
                                    Game.Instance.Log("Profile recieved from: " +
                                        NetUtility.ToHexString(msg.SenderConnection.RemoteUniqueIdentifier));
#endif
                                    if (pendingGamers.ContainsKey(msg.SenderConnection.RemoteUniqueIdentifier))
                                    {
                                        ...
                                        msg.ReadInt32();
                                        var gamerTag = msg.ReadString();
                                        msg.ReadInt32();
                                        msg.ReadInt32();
                                        var state = (GamerStates)msg.ReadInt32();
                                        state &= ~GamerStates.Local;
                                        var cgj = new CommandGamerJoined(msg.SenderConnection.RemoteUniqueIdentifier);
...
#if !WINDOWS_PHONE
                                        Game.Instance.Log(
                                            "We received a profile for an existing gamer.  Need to update it.");
#endif
                                    }
                                    break;
                                ...
                                case NetworkMessageType.GamerStateChange:
                                    var gamerstate = (GamerStates)msg.ReadInt32();
                                    gamerstate &= ~GamerStates.Local;
#if !WINDOWS_PHONE
                                    Game.Instance.Log("State Change from: " + msg.SenderEndpoint + " new State: " +
                                        gamerstate);
#endif
                                ...
                                case NetworkMessageType.SessionStateChange:
                                    var sessionState = (NetworkSessionState)msg.ReadInt32();
                                    ...
                                    foreach (var gamer in session.RemoteGamers)
                                    {
                                        ...
                                        {
#if !WINDOWS_PHONE
                                            Game.Instance.Log("Session State change from: " +
                                                NetUtility.ToHexString(msg.SenderConnection.RemoteUniqueIdentifier) +
        ...
        private void MGServer_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (online && availableSession == null)
            {
                // inform the master server we have closed
                var om = peer.CreateMessage();

                om.Write((byte)3);
                om.Write(session.Host.Gamertag);
                om.Write(peer.Configuration.AppIdentifier);
                peer.SendUnconnectedMessage(om, m_masterServer); // send message to peer
            }
        ...
        internal void DiscoverPeers() { peer.DiscoverLocalPeers(port); }

        internal void SendData(
            byte[] data,
            SendDataOptions options) { SendMessage(NetworkMessageType.Data, data, options, null); }

        internal void SendData(
            byte[] data,
            SendDataOptions options,
            NetworkGamer gamer) { SendMessage(NetworkMessageType.Data, data, options, gamer); }

        private void SendMessage(NetworkMessageType messageType, byte[] data, SendDataOptions options,
            NetworkGamer gamer)
        {
            var om = peer.CreateMessage();
        ...
        private void SendMessage(NetOutgoingMessage om, SendDataOptions options, NetworkGamer gamer)
        {
            ...
            var ndm = NetDeliveryMethod.Unreliable;
            switch (options)
            {
        ...
        private static void GetServerList(NetPeer netPeer)
        {
            ...
            var listRequest = netPeer.CreateMessage();
        ...
        internal static void FindResults(List<AvailableNetworkSession> networkSessions)
        {
            ...
            {
                var available = new AvailableNetworkSession();
                switch (im.MessageType)
                {
                    case NetIncomingMessageType.DiscoveryResponse:
                        var currentGameCount = im.ReadInt32();
                        var gamerTag = im.ReadString();
                        var openPrivateGamerSlots = im.ReadInt32();
                        var openPublicGamerSlots = im.ReadInt32();
                        var isHost = im.ReadBoolean();

                        var properties = new NetworkSessionProperties();
                        var propertyData = new int[properties.Count * 2];
                        for (var x = 0; x < propertyData.Length; x++)
                        {
                    ...
                    case NetIncomingMessageType.UnconnectedData:
                        if (im.SenderEndpoint.Equals(m_masterServer))
                        {
                            ...
                            var hostInternal = im.ReadIPEndpoint();
                            var hostExternal = im.ReadIPEndpoint();
        ...
        internal void UpdateLiveSession(NetworkSession networkSession)
        {
            if (peer != null && m_masterServer != null && networkSession.IsHost)
            {
                ...
                var adr = IPAddress.Parse(GetMyLocalIpAddress());
        ...
        internal bool IsReady { get { return peer != null; } }
    }
}
