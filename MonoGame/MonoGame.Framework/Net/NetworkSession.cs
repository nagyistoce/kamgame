...
namespace Microsoft.Xna.Framework.Net
{
    ...
    public sealed class NetworkSession : IDisposable
    {
        ...
        private NetworkSession() { activeSessions.Add(this); }

        ~NetworkSession() { Dispose(false); }

        private readonly NetworkSessionType sessionType;
        private readonly NetworkSessionProperties sessionProperties;
        ...
        private NetworkSession(NetworkSessionType sessionType, int maxGamers, int privateGamerSlots,
        ...
        private NetworkSession(NetworkSessionType sessionType, int maxGamers, int privateGamerSlots,
            NetworkSessionProperties sessionProperties, bool isHost, int hostGamer,
            AvailableNetworkSession availableSession) : this()
        {
            ...
            var gj = new CommandGamerJoined(hostGamer, this.isHost, true);
            ...
            )
        {
            ...
            {
                return
                    EndCreate(BeginCreate(sessionType, localGamers, maxGamers, privateGamerSlots, sessionProperties,
                        null, null));
            }
            finally {}
        }
...
#else
                return
                    EndCreate(BeginCreate(sessionType, maxLocalGamers, maxGamers, privateGamerSlots, sessionProperties,
                        null, null));
#endif
            }
            finally {}
            ...
            bool isHost)
        {
            ...
            try
            {
                ...
                session = new NetworkSession(sessionType, maxGamers, privateGamerSlots, sessionProperties, isHost,
                    hostGamer);
            }
            finally {}
            ...
            Object asyncState)
        {
            ...
            return BeginCreate(sessionType, hostGamer, 4, maxGamers, privateGamerSlots, sessionProperties, callback,
                asyncState);
        }
        ...
        public static IAsyncResult BeginCreate(
            ...
            int maxGamers,
            AsyncCallback callback,
            Object asyncState) { return BeginCreate(sessionType, -1, maxLocalGamers, maxGamers, 0, null, callback, asyncState); }
        ...
        public static IAsyncResult BeginCreate(
            ...
            Object asyncState)
        {
            return BeginCreate(sessionType, -1, maxLocalGamers, maxGamers, privateGamerSlots, sessionProperties,
                callback, asyncState);
        }
            ...
            Object asyncState)
        {
            ...
            try
            {
                NetworkSessionAsynchronousCreate AsynchronousCreate = Create;
                return AsynchronousCreate.BeginInvoke(sessionType, maxLocalGamers, maxGamers, privateGamerSlots,
                    sessionProperties, hostGamer, true, callback, asyncState);
            }
            finally {}
        }
        ...
        public static IAsyncResult BeginFind(
            ...
            NetworkSessionProperties searchProperties,
            AsyncCallback callback,
            Object asyncState) { return BeginFind(sessionType, -1, 4, searchProperties, callback, asyncState); }
            ...
            Object asyncState)
        {
            ...
            try
            {
                NetworkSessionAsynchronousFind AsynchronousFind = Find;
                return AsynchronousFind.BeginInvoke(sessionType, hostGamer, maxLocalGamers, searchProperties, callback,
                    asyncState);
            }
            finally {}
        }
            ...
            Object asyncState)
        {
            ...
            try
            {
                NetworkSessionAsynchronousJoin AsynchronousJoin = JoinSession;
            ...
            Object asyncState)
        {
            ...
            try
            {
                NetworkSessionAsynchronousJoinInvited AsynchronousJoinInvited = JoinInvited;
        ...
        public static AvailableNetworkSessionCollection Find(
            NetworkSessionType sessionType,
            int maxLocalGamers,
            NetworkSessionProperties searchProperties) { return EndFind(BeginFind(sessionType, -1, maxLocalGamers, searchProperties, null, null)); }
        ...
        private static NetworkSession JoinSession(AvailableNetworkSession availableSession)
        {
            ...
            try
            {
                var sessionType = availableSession.SessionType;
                var maxGamers = 32;
                var privateGamerSlots = 0;
                var isHost = false;
                var hostGamer = -1;
                var sessionProperties = availableSession.SessionProperties;
                if (sessionProperties == null)
                    sessionProperties = new NetworkSessionProperties();
                session = new NetworkSession(sessionType, maxGamers, privateGamerSlots, sessionProperties, isHost,
                    hostGamer, availableSession);
            }
            finally {}
        ...
        private void ProcessSendData(CommandSendData command)
        {
            ...
            NetworkGamer sender;
            var crd = new CommandReceiveData(command.sender.RemoteUniqueIdentifier,
                command.data);
        ...
        private void ProcessGamerLeft(CommandGamerLeft command)
        {
            ...
            for (var x = 0; x < _remoteGamers.Count; x++)
            {
        ...
        {
            var gamer = sender as NetworkGamer;
            if (gamer == null)
                return;
            ...
            if (gamer.IsLocal)
            {
                var sc = new CommandGamerStateChange(gamer);
                var cmd = new CommandEvent(sc);
        ...
        public GamerCollection<NetworkGamer> AllGamers { get { return _allGamers; } }

        private bool _AllowHostMigration;

        public bool AllowHostMigration
        {
            get { return _AllowHostMigration; }
            set
            {
        ...
        public int BytesPerSecondReceived { get { throw new NotImplementedException(); } }

        public int BytesPerSecondSent { get { throw new NotImplementedException(); } }

        public NetworkGamer Host { get { return hostingGamer; } }

        private bool _isDisposed;

        public bool IsDisposed
        {
            get { return _isDisposed; // TODO (this.kernelHandle == 0);
            }
        }
        ...
        public GamerCollection<LocalNetworkGamer> LocalGamers { get { return _localGamers; } }

        public int MaxGamers { get; set; }

        public GamerCollection<NetworkGamer> PreviousGamers { get { return _previousGamers; } }

        public int PrivateGamerSlots { get; set; }

        public GamerCollection<NetworkGamer> RemoteGamers { get { return _remoteGamers; } }

        public NetworkSessionProperties SessionProperties { get { return sessionProperties; } }

        public NetworkSessionState SessionState { get { return sessionState; } }

        public NetworkSessionType SessionType { get { return sessionType; } }
        ...
        internal static void Exit()
        {
            ...
            {
                foreach (NetworkSession session in activeSessions)
                {
    ...
    public class GamerJoinedEventArgs : EventArgs
    {
        ...
        public GamerJoinedEventArgs(NetworkGamer aGamer) { gamer = aGamer; }
    ...
    public class GamerLeftEventArgs : EventArgs
    {
        ...
        public GamerLeftEventArgs(NetworkGamer aGamer) { gamer = aGamer; }
    ...
    {
        private readonly NetworkGamer newHost;
        private readonly NetworkGamer oldHost;
        ...
        public NetworkGamer NewHost { get { return newHost; } }

        public NetworkGamer OldHost { get { return oldHost; } }
    }


    public class InviteAcceptedEventArgs : EventArgs
    {
        private readonly SignedInGamer gamer;

        public InviteAcceptedEventArgs(SignedInGamer aGamer) { gamer = aGamer; }

        public SignedInGamer Gamer { get { return gamer; } }

        public bool IsCurrentSession { get { return false; } }
    }


    public class NetworkSessionEndedEventArgs : EventArgs
    {
        private readonly NetworkSessionEndReason endReason;

        public NetworkSessionEndedEventArgs(NetworkSessionEndReason aEndReason) { endReason = aEndReason; }

        public NetworkSessionEndReason EndReason { get { return endReason; } }
    }
}