...
namespace Microsoft.Xna.Framework.Net
{
    public sealed class LocalNetworkGamer : NetworkGamer
    {
            ...
            lock (receivedData)
            {
                ...
                if (offset + crd.data.Length > data.Length)
                    throw new ArgumentOutOfRangeException("data",
                        "The length + offset is greater than parameter can hold.");
        ...
        public int ReceiveData(
            byte[] data,
            out NetworkGamer sender) { return ReceiveData(data, 0, out sender); }
        ...
        {
            var cme = new CommandEvent(new CommandSendData(data, offset, count, options, null, this));
        ...
        {
            var cme = new CommandEvent(new CommandSendData(data, offset, count, options, recipient, this));
            Session.commandQueue.Enqueue(cme);
        }
        ...
        public void SendData(
            ...
            SendDataOptions options)
        {
            var cme = new CommandEvent(new CommandSendData(data, 0, data.Length, options, null, this));
            Session.commandQueue.Enqueue(cme);
        }
        ...
        public void SendData(
            ...
            NetworkGamer recipient)
        {
            var cme = new CommandEvent(new CommandSendData(data, 0, data.Length, options, recipient, this));
            Session.commandQueue.Enqueue(cme);
        }
